...
var gulp = require('gulp'); // gulp core
  ...
  image = require('gulp-image'), // image compressor
  gulpFilter = require('gulp-filter'); // file filter
...
gulp.task('stylus', function () {
  var includesFilter = gulpFilter(function (file) {
    var first = file.path.substring(file.base.length)[0];
    return first != '_';
  });
...
gulp.task('sprite', function () {
  ...
  var spriteDataStyl = gulp.src('images/sprite/*.png').pipe(spritesmith({
    ...
    cssName: 'sprite.styl',
    padding: 10,
    cssOpts: {cssClass: function (item) {return '.sp-' + item.name;}}
  }));
  spriteData.img.pipe(gulp.dest('build/css/'));
  spriteDataStyl.css.pipe(gulp.dest('stylus/'));
});
...
gulp.task('images', function() {
  // compress images and create sprite
  gulp.run('sprite');
  gulp.run('image');
});
